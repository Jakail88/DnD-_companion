...
if missing_databases:
    ...
    1. **Posizionamento dei File**: Assicurati di aver caricato tutti i file di database `.json` direttamente nella cartella principale (**root**) del tuo repository GitHub, esattamente nello stesso livello in cui si trova il file `dnd_beyond_companion_v4.py`. Non metterli in cartelle separate se non strettamente necessario.
...
# Banner Display con risoluzione dinamica del percorso
script_dir = os.path.dirname(os.path.abspath(__file__))
...
if os.path.exists(banner_path):
    ...
    st.image("/workspace/artifacts/dnd_aesthetic_banner.jpg", use_column_width=True)
else:
