                    ...
                    for sub in race_info.get("subraces", []):
                        if isinstance(sub, dict):
                            ...
                            if sub_name:
                                ...
                                for st in sub.get("traits", []):
                                    if isinstance(st, dict):
                                        sub_traits_list.append(f"{st.get('name', '')}: {st.get('description', '')}")
                                    else:
                                        sub_traits_list.append(str(st))
...
if os.path.exists(banner_path):
    ...
    st.image("/workspace/artifacts/dnd_aesthetic_banner.jpg", use_container_width=True)
else:
