...
def export_character_pdf(char):
    ...
    char_name_esc = char['name'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").upper()
    story.append(Paragraph(f"D&D 5E CHARACTER SHEET: {char_name_esc}", title_style))
    total_lvl = sum(char["classes"].values())
    ...
    subclass_info_esc = subclass_info.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    race_esc = char['race'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    bg_esc = char['bg'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    align_esc = char['align'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    story.append(Paragraph(f"Livello {total_lvl} {active_class}{subclass_info_esc} &mdash; {race_esc} &mdash; {bg_esc} ({align_esc})", subtitle_style))
    ...
    headers_sum = ["Punti Ferita Max", "Classe Armatura (CA)", "Iniziativa", "Bonus Competenza (PB)", "Capacit&agrave; Carico"]
    ac_desc_short = ac_desc.split(' (')[0].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    rows_sum = [
        f"{char['max_hp']} PF",
        f"{ac_val} ({ac_desc_short})",
    ...
    t_summary.setStyle(TableStyle([
        ...
        ('GRID', (0, 0), (-1, -1), 0.8, pdf_colors['accent']),
    ...
    story.append(Paragraph("ABILIT&Agrave; E CARATTERISTICHE", h1_style))
    ...
    if atk_list:
        atk_hdrs = [
            ...
            Paragraph("<b>Propriet&agrave;</b>", th_style)
        ]
        atk_rows = []
        for atk in atk_list:
            atk_name_esc = atk['name'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            atk_ability_esc = atk['ability'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            atk_bonus_esc = atk['atk_bonus'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            atk_damage_esc = atk['damage'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            atk_properties_esc = atk['properties'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            
            atk_rows.append([
                Paragraph(f"<b>{atk_name_esc}</b>", tb_style),
                Paragraph(atk_ability_esc, tb_center),
                Paragraph(f"<b>{atk_bonus_esc}</b>", tb_center),
                Paragraph(f"<b>{atk_damage_esc}</b>", tb_center),
                Paragraph(atk_properties_esc, tb_style)
            ])
    ...
    if char["bg"] in bg_list_feats:
        ...
        bg_name_esc = char['bg'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        bg_privilege_esc = bg_privilege.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        bg_skills_esc = [s.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;") for s in bg_skills]
        bg_desc_esc = bg_desc.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        
        story.append(Paragraph(f"<b>Background: {bg_name_esc}</b>", ParagraphStyle('BgH', parent=body_style, fontName='Helvetica-Bold', textColor=pdf_colors['heading'])))
        story.append(Paragraph(f"&bull; <b>Privilegio:</b> {bg_privilege_esc}", body_style))
        story.append(Paragraph(f"&bull; <b>Competenze:</b> {', '.join(bg_skills_esc) if bg_skills_esc else 'Nessuna'}", body_style))
        if bg_desc_esc:
            story.append(Paragraph(f"&bull; <b>Descrizione:</b> {bg_desc_esc}", body_style))
        story.append(Spacer(1, 5))
            
    # Race details
    if char["race"] in RACES_DB:
        r_data = RACES_DB[char["race"]]
        race_name_esc = char['race'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        story.append(Paragraph(f"<b>Tratti della Stirpe ({race_name_esc}):</b>", ParagraphStyle('RcH', parent=body_style, fontName='Helvetica-Bold', textColor=pdf_colors['heading'])))
        for trait in r_data.get("traits", []):
            trait_esc = trait.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            story.append(Paragraph(f"&bull; {trait_esc}", body_style))
        story.append(Spacer(1, 5))
            
    # Class details
    class_name_esc = active_class.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    story.append(Paragraph(f"<b>Privilegi di Classe ({class_name_esc} Lvl {total_lvl}):</b>", ParagraphStyle('ClH', parent=body_style, fontName='Helvetica-Bold', textColor=pdf_colors['heading'])))
    ...
    if class_features:
        class_features_esc = [f.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;") for f in class_features]
        story.append(Paragraph(f"&bull; {', '.join(class_features_esc)}", body_style))
    else:
        story.append(Paragraph("&bull; Nessun privilegio sbloccato a questo livello.", body_style))
        
    story.append(Spacer(1, 10))
    
    # 6. Talents Section (Feats Expanded!)
    story.append(Paragraph("TALENTI ACQUISITI E DETTAGLI", h1_style))
    feats_acquired = char.get("feats", [])
    if feats_acquired:
        for f_name in feats_acquired:
            f_data = FEATS_DB.get("Feats", {}).get(f_name, {})
            f_name_esc = f_name.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            if f_data:
                prereq = f_data.get("prerequisites") or f_data.get("prereq") or "Nessuno"
                prereq_esc = prereq.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                
                benefits_raw = f_data.get("benefits") or f_data.get("desc") or "Nessun effetto."
                if isinstance(benefits_raw, list):
                    benefits_esc = "<br/>".join([f"&bull; {b.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')}" for b in benefits_raw])
                else:
                    benefits_esc = str(benefits_raw).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                
                story.append(Paragraph(f"<b>{f_name_esc}</b> (Prerequisiti: <i>{prereq_esc}</i>)", ParagraphStyle('FeatH', parent=body_style, fontName='Helvetica-Bold', textColor=pdf_colors['heading'], spaceBefore=4, spaceAfter=2)))
                story.append(Paragraph(benefits_esc, ParagraphStyle('FeatDesc', parent=body_style, leftIndent=12, spaceAfter=4)))
            else:
                story.append(Paragraph(f"<b>{f_name_esc}</b> (Talento personalizzato)", ParagraphStyle('FeatH', parent=body_style, fontName='Helvetica-Bold', textColor=pdf_colors['heading'], spaceBefore=4, spaceAfter=2)))
    else:
        story.append(Paragraph("<i>Nessun talento attivo selezionato (sbloccabili ai livelli ASI).</i>", body_style))
        
    story.append(Spacer(1, 10))
    
    # 7. Grimorio Spells Section (Grouped by level)
    story.append(Paragraph("GRIMORIO INCANTESIMI DI CLASSE", h1_style))
    class_spells = {}
    for sp_name, sp_data in SPELLS_DB.items():
        sp_classes = [c.lower() for c in sp_data.get("classes", [])]
        if active_class.lower() in sp_classes:
            try:
                sp_lvl = int(sp_data.get("level", 0))
            except:
                sp_lvl = 0
            class_spells.setdefault(sp_lvl, []).append((sp_name, sp_data))
            
    if class_spells:
        story.append(Paragraph(f"Lista completa degli incantesimi disponibili dalle espansioni per la classe <b>{class_name_esc}</b>:", body_style))
        for lvl in sorted(class_spells.keys()):
            lvl_name = "Trucchetti (Livello 0)" if lvl == 0 else f"Incantesimi di {lvl}° Livello"
            story.append(Paragraph(f"<b>{lvl_name}</b>", ParagraphStyle('SpLvlH', parent=body_style, fontName='Helvetica-Bold', textColor=pdf_colors['heading'], spaceBefore=6, spaceAfter=2)))
            
            for sp_name, sp_data in sorted(class_spells[lvl], key=lambda x: x[0]):
                school = sp_data.get("school", "")
                casting_time = sp_data.get("casting_time") or sp_data.get("time", "1 Azione")
                sp_range = sp_data.get("range", "Personale")
                components = sp_data.get("components", "V, S")
                duration = sp_data.get("duration", "Istantanea")
                desc = sp_data.get("description") or sp_data.get("desc", "")
                
                sp_name_esc = sp_name.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                school_esc = school.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                casting_time_esc = casting_time.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                sp_range_esc = sp_range.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                duration_esc = duration.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                desc_esc = desc.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                
                sp_header = f"&bull; <b>{sp_name_esc}</b> (<i>{school_esc}</i>) &mdash; Tempo: {casting_time_esc} | Gittata: {sp_range_esc} | Durata: {duration_esc}"
                story.append(Paragraph(sp_header, body_style))
                story.append(Paragraph(desc_esc, ParagraphStyle('SpDesc', parent=body_style, fontSize=8.5, leftIndent=12, textColor=pdf_colors['muted'], spaceAfter=3)))
    else:
        story.append(Paragraph(f"La classe <b>{class_name_esc}</b> non possiede incantesimi registrati o non è una classe incantatrice.", body_style))
        
    story.append(Spacer(1, 10))
    
    # 8. Remaining Inventory
    story.append(Paragraph("ALTRO EQUIPAGGIAMENTO NELLO ZAINO", h1_style))
    non_weapons_inv = []
    for item in char["inventory"]:
        base_name = item.split(" (")[0] if " (" in item else item
    ...
    if non_weapons_inv:
        non_weapons_esc = [i.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;") for i in non_weapons_inv]
        story.append(Paragraph(f"<b>Altri oggetti nello Zaino:</b> {', '.join(non_weapons_esc)}", body_style))
    else:
...
with m_tab1:
    ...
    # ------------------------------------------
    # CHARACTER SUMMARY BAR (D&D Beyond Style)
    # ------------------------------------------
    total_lvl_sum = sum(char["classes"].values())
    pb_sum = get_pb(total_lvl_sum)
    ac_val_sum, ac_desc_sum = compute_ac(char)
    dex_mod_sum = get_modifier(char["stats"].get("DEX", 10))
    init_bonus_sum = f"+{dex_mod_sum}" if dex_mod_sum >= 0 else str(dex_mod_sum)
    
    # Dynamic Speed Calculation
    speed_base_sum = RACES_DB.get(char["race"], {}).get("speed", 30)
    tot_w_sum = 0.0
    for item in char["inventory"]:
        item_weight = 1.0
        if item in EQUIP_DB.get("Armor", {}):
            ...
            try:
                ...
                item_weight = 1.0
        tot_w_sum += item_weight
    max_c_sum, _ = compute_carrying_capacity(char)
    pct_sum = tot_w_sum / max_c_sum if max_c_sum > 0 else 0.0
    if pct_sum >= 1.0:
        speed_val_sum = 5
    elif pct_sum > 0.66:
        speed_val_sum = max(speed_base_sum - 10, 5)
    else:
        speed_val_sum = speed_base_sum

    # Layout row for metrics
    col_sum1, col_sum2, col_sum3, col_sum4, col_sum5 = st.columns(5)
    with col_sum1:
        st.markdown(f"""
        <div style='text-align: center; background-color: #2e1d11; border: 2px solid #8c1919; border-radius: 8px; padding: 10px; box-shadow: inset 0 0 6px rgba(140, 25, 25, 0.2);'>
            <div style='font-size: 0.75rem; font-weight: bold; color: #b88d40; text-transform: uppercase; white-space: nowrap;'>CLASSE ARMATURA</div>
            <div style='font-size: 1.8rem; font-weight: bold; color: #e6dfc8; margin: 4px 0;'>{ac_val_sum}</div>
            <div style='font-size: 0.7rem; color: #c9bfab; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;' title='{ac_desc_sum}'>{ac_desc_sum}</div>
        </div>
        """, unsafe_allow_html=True)
    with col_sum2:
        st.markdown(f"""
        <div style='text-align: center; background-color: #2e1d11; border: 2px solid #8c1919; border-radius: 8px; padding: 10px; box-shadow: inset 0 0 6px rgba(140, 25, 25, 0.2);'>
            <div style='font-size: 0.75rem; font-weight: bold; color: #b88d40; text-transform: uppercase; white-space: nowrap;'>PUNTI FERITA</div>
            <div style='font-size: 1.8rem; font-weight: bold; color: #e6dfc8; margin: 4px 0;'>{char['current_hp']} / {char['max_hp']}</div>
            <div style='font-size: 0.7rem; color: #c9bfab;'>HP Massimi</div>
        </div>
        """, unsafe_allow_html=True)
    with col_sum3:
        st.markdown(f"""
        <div style='text-align: center; background-color: #2e1d11; border: 2px solid #8c1919; border-radius: 8px; padding: 10px; box-shadow: inset 0 0 6px rgba(140, 25, 25, 0.2);'>
            <div style='font-size: 0.75rem; font-weight: bold; color: #b88d40; text-transform: uppercase; white-space: nowrap;'>INIZIATIVA</div>
            <div style='font-size: 1.8rem; font-weight: bold; color: #e6dfc8; margin: 4px 0;'>{init_bonus_sum}</div>
            <div style='font-size: 0.7rem; color: #c9bfab;'>Modificatore DES</div>
        </div>
        """, unsafe_allow_html=True)
    with col_sum4:
        st.markdown(f"""
        <div style='text-align: center; background-color: #2e1d11; border: 2px solid #8c1919; border-radius: 8px; padding: 10px; box-shadow: inset 0 0 6px rgba(140, 25, 25, 0.2);'>
            <div style='font-size: 0.75rem; font-weight: bold; color: #b88d40; text-transform: uppercase; white-space: nowrap;'>VELOCITÀ</div>
            <div style='font-size: 1.8rem; font-weight: bold; color: #e6dfc8; margin: 4px 0;'>{speed_val_sum} ft</div>
            <div style='font-size: 0.7rem; color: #c9bfab;'>Base: {speed_base_sum} ft</div>
        </div>
        """, unsafe_allow_html=True)
    with col_sum5:
        st.markdown(f"""
        <div style='text-align: center; background-color: #2e1d11; border: 2px solid #8c1919; border-radius: 8px; padding: 10px; box-shadow: inset 0 0 6px rgba(140, 25, 25, 0.2);'>
            <div style='font-size: 0.75rem; font-weight: bold; color: #b88d40; text-transform: uppercase; white-space: nowrap;'>PROF. BONUS</div>
            <div style='font-size: 1.8rem; font-weight: bold; color: #e6dfc8; margin: 4px 0;'>+{pb_sum}</div>
            <div style='font-size: 0.7rem; color: #c9bfab;'>Livello {total_lvl_sum}</div>
        </div>
        """, unsafe_allow_html=True)
